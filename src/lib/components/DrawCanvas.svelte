<script lang="ts">
	...
	export let originalWidth = 100;
	export let originalHeight = 100;
	...
	function scaleCanvas() {
		for (let i = 0; i < canvasByLayer.length; i++) {
			let dimensions = getObjectFitSize(true, canvasByLayer[i].clientWidth, canvasByLayer[i].clientHeight, canvasByLayer[i].width, canvasByLayer[i].height);
			const dpr = window.devicePixelRatio || 1;
			canvasByLayer[i].width = dimensions.width * dpr;
			canvasByLayer[i].height = dimensions.height * dpr;
		}
		renderFull();
	}
...
<svelte:window on:resize={scaleCanvas}></svelte:window>

<section>
	<div class="canvas-wrapper">
		<canvas class="sketch" bind:this={canvasByLayer[2]} width={originalWidth} height={originalHeight}></canvas>
		<canvas class="above" bind:this={canvasByLayer[1]} width={originalWidth} height={originalHeight}></canvas>
		<canvas class="above" bind:this={canvasByLayer[0]} width={originalWidth} height={originalHeight}></canvas>
	</div>

	{#if canDraw}
		<input type="color" bind:value={selectedColor}>
		<span>Brush size</span>
		<input type="range" min="0.002" max="0.05" bind:value={brushSize} step="0.001">
