<script lang="ts">
	...
	onMount(() =>
	{
		...
		if (params != undefined && params.partyId != undefined)
			Global.partyId = params.partyId;
		...
		Global.socket.on('connect', () =>
		{
			...
			if (Global.partyId == null)
			{
				start = () =>
				{
					Global.socket.on('partyCreated', (partyId: string) =>
					{
						Global.partyId = partyId;
						window.location.href = '#/party';
					});

					Global.socket.emit('createParty', image);
				}
			}
			...
			else
			{
				start = () =>
				{
					Global.socket.on('partyJoined', () =>
					{
						window.location.href = "#/party";
					});

					Global.socket.emit('joinParty', Global.partyId, image);
				}

				Global.socket.emit('checkParty', Global.partyId);
			}
		});
	});
	...
	onDestroy(() =>
	{
		Global.socket.off('partyCreated');
		Global.socket.off('partyJoined');
	});
</script>
...
<main class="flex justify-center items-center fixed">
	...
	<div class="start flex justify-center items-center flex-col">
		...
		<PartyButton onClick={start} type={Global.partyId == null ? 'create' : 'join'}/>
	</div>
</main>
